...
public class ResourceRepository implements ResourceDAO
{
    ...
    public boolean updateLibraryResource(LibraryResource resource) throws Exception{
        ...
        boolean specificResourceUpdated = false;
        ...
        if (resource instanceof Book) {
            ...
            try (CallableStatement callableStatement = connection.prepareCall(sql)) {
                ...
                callableStatement.setDate(5, Date.valueOf(((Book) resource).getPublicationDate()));

                specificResourceUpdated = callableStatement.executeUpdate() > 0;
            }
        } else if (resource instanceof Journal) {
            String sql = "{call updateJournal(?,?,?)}";
            try (CallableStatement callableStatement = connection.prepareCall(sql)) {
                callableStatement.setLong(1, resource.getResourceId());
        ...
        // Update shared attributes (LibraryResource)
        String updateResourceSql = "{call updateResource(?,?,?)}";
        try (CallableStatement callableStatement = connection.prepareCall(updateResourceSql)) {
            callableStatement.setLong(1, resource.getResourceId());
            callableStatement.setString(2, resource.getTitle());
            callableStatement.setString(3, resource.getResourceStatus().toString());

            resourceUpdated = callableStatement.executeUpdate() > 0;
        }

        return resourceUpdated && specificResourceUpdated;
